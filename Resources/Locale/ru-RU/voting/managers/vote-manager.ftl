...
ui-vote-votekick-title = { $initiator }  вызвал голосование для пользовател: { $targetEntity }. Причина: { $reason }
ui-vote-votekick-success = Голосование за { $target } преуспевший. Выберите причину голосования: { $reason }
ui-vote-votekick-server-cancelled = Голосование за { $target } было отменено сервером.
ui-vote-votekick-not-enough-eligible = Недостаточно онлайн-избирателей, имеющих право голоса, для начала голосования: { $voters }/{ $requirement }
