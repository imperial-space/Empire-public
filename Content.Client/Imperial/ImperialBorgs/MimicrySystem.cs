...
namespace Content.Client.Borgs
{
    public sealed class MimicrySystem : EntitySystem
    {
        ...
        private void OnAppearanceChange(EntityUid uid, MimicryComponent component, ref AppearanceChangeEvent args)
        {
            if (args.Sprite == null)
                return;
            if (_appearance.TryGetData<bool>(uid, ImperialBorgVisuals.Eng, out var eng, args.Component) && eng)
            {
            ...
            else if (_appearance.TryGetData<bool>(uid, ImperialBorgVisuals.Real, out var real, args.Component) && real)
            {
            ...
            // else if (_appearance.TryGetData<bool>(uid, ImperialBorgVisuals.Dead, out var dead, args.Component) && dead)
            // {
            //     args.Sprite.LayerSetState(0, component.DeadState);
            //     args.Sprite.LayerSetState(BorgVisualLayers.Light, false);
